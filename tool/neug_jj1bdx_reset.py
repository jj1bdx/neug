...
class neug(object):
    ...
    def reset_neug(self):
        self.__devhandle.controlMsg(requestType = 0x40, request = 250,
                                    buffer = "poi", value = 0, index = 0,
                                    timeout = 1000)
        return
...
def main():
    ...
    for (dev, config, intf) in com_devices():
        try:
            ...
            print("usbVersion: %s" % dev.usbVersion)
            print("idVendor: %s" % hex(dev.idVendor))
            print("idProduct: %s" % hex(dev.idProduct))
